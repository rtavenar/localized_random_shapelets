...
# Models
model_ssgl = ssgl_linear_regression(dim_input=dim,
                                    groups=groups,
...
model_lasso = ssgl_linear_regression(dim_input=dim,
                                     groups=None,
                                     indices_sparse=numpy.array([1] * dim),
                                     alpha=1.,
                                     lbda=.01)
...
model_sgl = ssgl_linear_regression(dim_input=dim,
                                   groups=groups,
                                   indices_sparse=numpy.array([1] * dim),
                                   alpha=.5,
                                   lbda=.01)
...
beta_hat_ssgl = numpy.concatenate((beta_hat_ssgl[:2],
                                   beta_hat_ssgl[4:],
                                   beta_hat_ssgl[2:4]))
beta_hat_lasso = numpy.concatenate((beta_hat_lasso[:2],
                                    beta_hat_lasso[4:],
                                    beta_hat_lasso[2:4]))
beta_hat_sgl = numpy.concatenate((beta_hat_sgl[:2],
                                  beta_hat_sgl[4:],
                                  beta_hat_sgl[2:4]))
...
plt.bar(ind+width, numpy.abs(beta_hat_ssgl), width,
        ...
        lw=2,
        alpha=0.6)
plt.bar(ind+2*width, numpy.abs(beta_hat_lasso), width,
        label="Lasso (MSE=%.2f)" % mse_lasso,
        lw=2,
        alpha=0.6)
plt.bar(ind+3*width, numpy.abs(beta_hat_sgl), width,
